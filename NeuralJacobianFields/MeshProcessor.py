...
class MeshProcessor:
    ...
    def get_writeable(self):
        ...
        return {key: value for key, value in out_np.items() if value is not None}, {key: value for key, value in
                                                                                    out_npz.items() if
                                                                                    value is not None}
    ...
    def sample_points(self, n):
        ...
        point_samples = self.vertices[vert_ind[:, 0]] * bary[:, 0:1] + self.vertices[vert_ind[:, 1]] * bary[:, 1:2] + \
                        self.vertices[vert_ind[:, 2]] * bary[:, 2:3]
        normal_samples = self.normals[vert_ind[:, 0]] * bary[:, 0:1] + self.normals[vert_ind[:, 1]] * bary[:, 1:2] + \
                         self.normals[vert_ind[:, 2]] * bary[:, 2:3]
        wks_samples = None
        if self.load_wks_centroids or self.load_wks_samples:
            wks_samples = self.vert_wks[vert_ind[:, 0]] * bary[:, 0:1] + self.vert_wks[vert_ind[:, 1]] * bary[:, 1:2] + \
                          self.vert_wks[vert_ind[:, 2]] * bary[:, 2:3]
...
class WaveKernelSignature:
    ...
    def compute(self):
        ...
        K = np.matmul(np.square(self.eig_vecs[:, 1:]),
                      coef)  # VxT. Scaling of the eigen vectors by coef. Coef depends only on the eigen values. Triangulation agnostic.
        self.wks = K / np.expand_dims(sum_coef,
                                      0)  # VxT Scaling of the eigen vectors by sum_coef. Coef depends only on the eigen values. Triangulation agnostic.
