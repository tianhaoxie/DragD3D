...
class SourceMesh:
    ...
    def __init__(self, source_ind, source_dir, extra_source_fields,
        ...
        self.mask = mask
        self.source_mesh_masked_centroid = None
        self.poisson = None
    ...
    def vertices_from_jacobians(self, d):
        return self.poisson.solve_poisson(d, self.mask, self.source_mesh_masked_centroid)
    ...
    def __init_from_mesh_data(self):
        ...
        if self.mask.any():
            source_vertice_masked = self.source_vertices.numpy()[self.mask]
            bb = igl.bounding_box(source_vertice_masked)[0]
            self.source_mesh_masked_centroid = (bb[0] + bb[-1]) / 2

        if self.random_centering:
            # centering augmentation
            self.source_mesh_centroid = self.source_mesh_centroid + [(2 * random.random() - 1) * diag * 0.2,
                                                                     (2 * random.random() - 1) * diag * 0.2,
                                                                     (2 * random.random() - 1) * diag * 0.2]
    ...
    def load(self, source_v=None, source_f=None):
        ...
        if source_v is not None and source_f is not None:
            self.mesh_processor = MeshProcessor.MeshProcessor.meshprocessor_from_array(source_v, source_f,
                                                                                       self.source_dir, self.__ttype,
