...
USE_TORCH_SPARSE = False  ## This uses TORCH_SPARSE instead of TORCH.SPARSE
...
USE_SCIPY = True  ## This uses CUPY LU decomposition on CPU

# If USE_SCIPY = True, whether or not to use enhanced backend
...
class PoissonSystemMatrices:
    ...
    Logic : this class is supposed  to hold everything needed to compute Poisson Solver
    ...
    def compute_poisson_solver_from_laplacian(self, compute_splu=True):
        ...
        if compute_splu:
            self.compute_splu()
        return self.create_poisson_solver_from_splu(self.__splu_L, self.__splu_U, self.__splu_perm_c,
                                                    self.__splu_perm_r)
...
class PoissonSolver:
    ...
    def solve_poisson(self, jacobians, mask = np.array([]), masked_source_centroid=None):
        # st = time.time()
        ...
        if mask.any():
            mask = torch.from_numpy(mask).cuda().squeeze()
            masked_sol = torch.index_select(sol,1, mask)
            v_max, _ = torch.max(masked_sol, dim=1)
            v_min, _ = torch.min(masked_sol, dim=1)
            masked_c = (v_max+v_min).unsqueeze(1)/2
            c = masked_c - torch.from_numpy(masked_source_centroid).to(sol.device)
        else:
            v_max,_ = torch.max(sol,dim=1)
            v_min,_ = torch.min(sol,dim=1)
            c = (v_max+v_min).unsqueeze(1)/2
...
def poisson_system_matrices_from_mesh(V, F, dim=3, ttype=torch.float64, is_sparse=True, cpuonly=False):
    ...
    if is_sparse:
        ...
        rhs = rhs.toarray()
        grad = grad.toarray()
    grad = _convert_sparse_igl_grad_to_our_convention(grad)
    grad = SparseMat.from_M(grad, torch.float64)
