...
def weighted_perpendicular_aggregator(delta_noise_preds, weights, batch_size):
    ...
    for i, complementary_noise_pred in enumerate(delta_noise_preds[1:], start=1):
        ...
        if sum(idx_non_zero) == 0:
            continue
        accumulated_output[idx_non_zero] += weights[i][idx_non_zero].reshape(-1, 1, 1,
                                                                             1) * batch_get_perpendicular_component(
            complementary_noise_pred[idx_non_zero], main_positive[idx_non_zero])
