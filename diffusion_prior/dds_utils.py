...
class StableDiffusion(nn.Module):
    def __init__(self, device, fp16, vram_O, sd_version='2.1', hf_key=None, t_range=[0.02, 0.98]):
        ...
        #self.unet = torch.compile(pipe.unet,mode="reduce-overhead", fullgraph=True)
        self.unet = pipe.unet
    ...
    def pred_noise_map(self, latents, noise, t, guidance_scale, text_embeddings):
        ...
        tt = torch.cat([t] * 2)
        noise_pred = self.unet(latent_model_input, tt, encoder_hidden_states=text_embeddings)[0]
    ...
    def train_step(self, text_embeddings, pred_rgb, ref_rgb, guidance_scale=100, as_latent=False, grad_scale=1):
    ...
    def pred_noise_map_perpneg(self, latents, noise, t, guidance_scale, text_embeddings, weights,  B ,K):
        ...
        tt = torch.cat([t] * (1 + K))
        unet_output = self.unet(latent_model_input, tt, encoder_hidden_states=text_embeddings, return_dict= False)[0]
